...
def run_face_recognition():
    ...
    while True:
        ...
        # If recording is turned on, control the frame rate
        if recording:
            key = cv2.waitKey(42)  # Adjust the delay to control the frame rate (42 milliseconds for approximately 24 fps)
            if key == 27:
                recording = False
                if video_writer:
        ...
        for i, (x, y, w, h) in enumerate(faces):
            ...
            # Draw a rectangle around the detected face
            if intruder_name:
                print(f"INTRUDER: {intruder_name}")
